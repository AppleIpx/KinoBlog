...
from some_proj.serials.tests.utils.create_serials import create_serial
...
class TestSerialSignals(TestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.serial = create_serial()

    def test_media_file_creation_on_post_save(self):
        self.assertEqual(MediaFile.objects.count(), 1)
        media_file_film = MediaFile.objects.get(object_id=self.serial.id)
        self.assertEqual(media_file_film.content_type, ContentType.objects.get_for_model(SerialModel))
    ...
    def test_pre_save_signal_for_media_file(self, mock_start_signal_processes):
        media_file = create_media(SerialModel, self.serial.id)
    ...
    def test_pre_save_signal_saves_updated_orig_path_file(self, mock_start_signal_processes):
        self.media_file = create_media(SerialModel, self.serial.id)
